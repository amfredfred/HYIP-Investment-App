    ...
    <div class="confirm-deposit">
        <div class="hash-form">
            <div v-if="message.length > 0 " class="mb-4">
                <sva-alert v-for="(mes, index) in message" :key="index" :message="mes" :type="message_type"></sva-alert>
            </div>
            <form method="post" @submit.prevent="try_confirm_deposit($event.target)">
                ...
                <div class="form-group mb-4">
                    <button class="btn btn-primary px-4" :disabled="confirming_deposit">
                        Confirm Deposit
                        <i class="fa fa-circle-o-notch fa-spin text-light" v-if="confirming_deposit" role="status">
